...
import { Mail, Lock, ArrowRight, ShieldCheck, Zap, Globe2, Loader2 } from 'lucide-react';
...
export default function Login() {
  ...
  const submit = (e?: React.FormEvent) => {
    ...
    if (!selected) { setErr('This workspace is private — use one of the two partner accounts.'); return; }
    setBusy(true);
    window.setTimeout(() => {
      login(selected.id);
      nav('/dashboard');
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="w-full max-w-md mx-auto">
            ...
            <form onSubmit={submit} className="mt-5 space-y-3">
              ...
              <Btn type="submit" disabled={busy} className="h-12! w-full text-[14px]! rounded-xl!" icon={busy ? Loader2 : ArrowRight}>
                {busy ? 'Opening your workspace…' : 'Enter workspace'}
              </Btn>
              <p className="text-[11px] text-center text-[#5d6880]">Demo workspace — any password works. Data stays in your browser.</p>
            </form>
